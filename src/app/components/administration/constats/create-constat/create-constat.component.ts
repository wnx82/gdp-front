  ...
  getGeolocation() {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(position => {
            ...
            this.http
                .get(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}&key=${environment.apiGoogleMapKey}`)
                .subscribe((data: any) => {
                    this.location = data.display_name;
                    // console.log(this.location);

                    if (this.location) {
                        const locationArray = this.location.split(',').map(item => item.trim());
                        ...
                        if (locationArray.length >= 2) {
                            ...
                            console.error('Adresse inattendue, moins de 2 parties:', locationArray);
                        }
                    } else {
    ...
    findAndSelectRue(locationJson: any) {
        ...
        const normalizedNomComplet = locationJson.nomComplet.trim().toLowerCase();

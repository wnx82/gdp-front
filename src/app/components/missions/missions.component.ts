...
export class MissionsComponent implements OnInit {
  ...
  getMissions() {
    this.http.get<Mission[]>(this.BASE_URL).pipe(
  ...
  addMission(mission: Mission) {
    this.http.post<Mission>(this.BASE_URL, mission).pipe(
  ...
  editMission(mission: Mission) {
    this.http.put<Mission>(`${this.BASE_URL}${mission.id}`, mission).pipe(
      map(data => {
        const index = this.missions.findIndex(m => m.id === data.id);
